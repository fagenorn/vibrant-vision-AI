...
class IFBlock(nn.Module):
    ...
    def forward(self, x, flow=None, scale=1):
        ...
        if flow is not None:
            flow = F.interpolate(flow, scale_factor=1.0 / scale, mode="bilinear") * 1.0 / scale
...
class IFNet(nn.Module):
    ...
    def forward(self, img0, img1, timestep):
        ...
        for i in range(4):
            if flow is None:
                flow, mask = block[i](
                    torch.cat((img0[:, :3], img1[:, :3], timestep), 1), None, scale=self.scale_list[i]
                )
                if self.ensemble:
                    f1, m1 = block[i](
                        torch.cat((img1[:, :3], img0[:, :3], 1 - timestep), 1), None, scale=self.scale_list[i]
                    )
